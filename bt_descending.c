...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/02/04 14:16:33 by ahugh             #+#    #+#             */
/*   Updated: 2019/02/04 16:58:28 by ahugh            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		apply_ins(t_list **stack, t_list **ins, int index)
{
	...
	{
		if (index == 0)
			state = ins_swap(ins, 0, stack, "sb");
		else if (index == 1)
			state = ins_rotate(ins, 0, stack, "rb");
		else if (index == 2)
			state = ins_rev_rotate(ins, 0, stack, "rrb");
	}
	return (state);
}
...
void		bt_descending(t_list **stack, t_list **ins, size_t size)
{
	...
	iter = 1;
	new = 0;
	while (!(stack_descending(*stack, size)))
